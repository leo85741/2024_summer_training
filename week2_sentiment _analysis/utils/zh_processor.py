...
def sentence_segmenation(df):
    ...
    # 以標點符號斷句
    df['sentence']=df['sentence'].apply(lambda x:split_sentence(x))
...
def tokenization(df, user_dict_path=None):
    ...
    # 5. 使用jieba進行斷詞
    df['sentence'] = df['sentence'].apply(lambda x: list(jieba.cut(re.sub(r'[^\w\s]', '', x))))
    df=df.explode('sentence').reset_index(drop=True)
...
def remove_stopword(df, stopword):
    ...
    df['word'] = df['sentence'].apply(lambda x: ''.join([word for word in x.split() if word not in stopword]))
